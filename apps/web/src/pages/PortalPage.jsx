...
const ENTRY_META = [
  ...
  },
  {
    ...
    icon: <ShopOutlined />,
    description: "查看天猫奥莱、奥莱安建立、C店货盘明细，支持查询和导出。",
  },
  {
    key: "arrival",
    permission: "arrival",
...
export default function PortalPage() {
  ...
  async function rebuildWeeklyData() {
    ...
    try {
      ...
      setPipelineStatus({
        type: "success",
        text: "销售与库存报表同步完成。日报、奥莱货盘、分析和看板页面重新请求接口后就会显示最新数据。",
      });
  ...
  const entries = useMemo(() => {
    const next = ENTRY_META
      .filter((item) => hasPermission(item.permission || item.key))
      .map((item) => ({
        ...item,
      }));
    if (isAdmin) {
      ...
      {isAdmin ? (
        ...
        >
          <Space direction="vertical" size={12} style={{ width: "100%" }}>
            <Text type="secondary">
              把 `data/inbox` 里的最新库存和销售文件导入报表数据库。日报、奥莱货盘、分析、可视化和渠道页面读取的是数据库结果，不会直接读取原始文件。
            </Text>
            <Alert
              showIcon
