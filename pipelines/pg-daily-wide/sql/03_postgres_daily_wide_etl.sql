...
truncate table anta_daily.rpt_daily_sku_wide;
...
inventory_mapped as (
    ...
    where position('u' in lower(i.sku)) = 0
      and position('v' in lower(i.sku)) = 0
    group by
        i.snapshot_date,
        btrim(i.sku),
...
insert into anta_daily.rpt_daily_sku_wide (
    ...
    style_discount_total, inventory_snapshot_date
)
select
    sp.sales_date,
    pd.style,
    ...
    pd.gender,
    pd.story_pack,
    coalesce(ip.inv_huotong_qty, 0), coalesce(ip.inv_women_qty, 0), coalesce(ip.inv_outdoor_qty, 0),
    coalesce(ip.inv_trend_qty, 0), coalesce(ip.inv_casual_qty, 0), coalesce(ip.inv_c_store_qty, 0),
    coalesce(ip.inv_category_shared_qty, 0), coalesce(ip.inv_tmall_outlet_qty, 0), coalesce(ip.inv_shared_qty, 0),
    coalesce(ip.inv_tmall_flagship_qty, 0), coalesce(ip.inv_tmall_franchise_qty, 0),
    coalesce(ip.inv_shanghai_franchise_qty, 0), coalesce(ip.inv_jd_flagship_qty, 0),
    coalesce(ip.inv_jd_franchise_qty, 0), coalesce(ip.inv_jd_self_qty, 0), coalesce(ip.inv_dewu_qty, 0),
    coalesce(ip.inv_interest_qty, 0), coalesce(ip.inv_vip_qty, 0), coalesce(ip.inv_pdd_qty, 0),
    coalesce(ip.inv_distributor_qty, 0), coalesce(ip.inv_other_qty, 0), coalesce(ip.inventory_total_qty, 0),
